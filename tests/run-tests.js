...
console.log('====================================================');
console.log('KGM CINEMATIC STUDIO AUTOMATED TEST SUITE');
...
runTest('Motion Library has 10+ standard presets with architectural safety', () => {
  const db = new Database(DB_PATH);
  const presets = db.prepare('SELECT * FROM motion_presets').all();
...
runTest('Image Analysis maps room names to correct categories', () => {
  const { ImageAnalysisService } = require('../lib/ai/image-analysis');
...
runTest('Cinematic Prompt Engine strictly enforces architectural preservation', () => {
  const { CinematicPromptEngine } = require('../lib/ai/prompt-engine');
  const dummyPreset = {
...
runTest('Storyboard Assistant organizes scenes into natural luxury walkthrough', () => {
  const { StoryboardAssistant } = require('../lib/ai/storyboard-assistant');
  const dummyImages = [
...
runTest('AI Film Director Quality Control detects readiness correctly', () => {
  const { FilmDirectorReviewEngine } = require('../lib/ai/film-director-review');
  const db = new Database(DB_PATH);
...
runTest('Voiceover Engine generates accurate multi-lingual scripts', () => {
  const { VoiceoverEngine } = require('../lib/audio/voiceover-engine');
  const dummyProp = {
...
runTest('RBAC correctly enforces permission hierarchy across all 6 roles', () => {
  const { AuthService, ROLE_PERMISSIONS } = require('../lib/security/auth');
